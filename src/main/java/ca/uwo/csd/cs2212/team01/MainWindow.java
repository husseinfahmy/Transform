...
public class MainWindow extends JFrame implements Serializable {
     ...
     * @return
     */
    public User getUser() { return this.userPreferences.getUser(); }
    /**
     * Sets the user object.
     * @param user
     */
    public void setUser(User user) { this.userPreferences.setUser(user); }

    /**
     * Gets the time a last API call was made.
     * @return
     */
    public Date getLastCall() { return this.userPreferences.getLastCall(); }
    /**
     * Sets the time the last API call was made.
     * @param lastCall
     */
    public void setLastCall(Date lastCall) { this.userPreferences.setLastCall(lastCall); }
    
    /**
     * Checks if the API call is the first time in real-time.
     * @return
     */
    public boolean isFirstCall() { return this.userPreferences.getFirstCall(); }
	/**
	 * Sets the first API call time in real-time.
	 * @param firstCall
	 */
	public void setFirstCall(boolean firstCall) { this.userPreferences.setFirstCall(firstCall); }

	/**
	 * Returns this week's data.
	 * @return
	 */
	public LinkedList<Day> getDays() { return this.userPreferences.getDays(); }
    /**
	...
	public Feedback updateWeeklyProgress() //to be called once everyday, every morning?
	{
		...
		for(int i = 6; i>0;i--) past6Days.add(this.userPreferences.getDays().get(this.userPreferences.getDays().size()-1-i));
	 ...
	 */
	public VirtualTrainer getVirtualTrainer() { return this.userPreferences.getVt(); }
	/**
	 * Sets the Virtual Trainer object
	 * @param vt
	 */
	public void setVirtualTrainer(VirtualTrainer vt) { this.userPreferences.setVt(vt); } 
	 ...
	 * @return
	 */
	public NavigationScreen getNavigationScreen() { return this.navScreen; }
	/**
	 ...
	 * @return
	 */
	public ContinueScreen getContinueScreen() { return this.continueScreen; }
	/**
	 * Gets the Setup Screen object.
	 * @return
	 */
	public SetupScreen getSetupScreen() { return this.setupScreen; }
	/**
	 * Gets the Add Meal and Dishes Screen object.
	 * @return
	 */
	public AddMealDishScreen getAddMealDishScreen() { return this.addMealDishScreen; }
	/**
    ...
    private void createFonts() {
    	...
    	//Font font = null;
		try {
			FONT_HELVETICA_NEUE_THIN = Font.createFont(Font.TRUETYPE_FONT, new File("src/main/resources/FONTS/HelveticaNeueThin.ttf"));
		...
		try {
			FONT_HELVETICA_NEUE_ITALIC = Font.createFont(Font.TRUETYPE_FONT, new File("src/main/resources/FONTS/HelveticaNeueThinItalic.ttf"));
		} catch (FontFormatException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (IOException e) {
	...
	/**
	 * Setup and call the API method in a thread which retrieves data from the FitBit API on a day-to-day basis.
	 * @param date
	 * @param startTime
	 * @param endTime
	 * @param day
	...
	/**
	 * Setup and call the API method in a thread which stores the data into data structures (arrays) from the API call.
	 * @param responseBody
	 * @param name
	 * @param day
	 */
	...
	public void refreshEvent()
	{
			...
			long timeRange = (currentTime.getTime() - this.userPreferences.getLastCall().getTime())/(long)1000/(long)60; 		//convert time difference to minutes
			//////////////////////////////
			...
			if(timeRange > 0) 																				//else: not enough time has elapsed to warrant a refresh
			{
				...
				if (timeRange < todayRemainder) 															//Update current day up till currentTime
				{
					...
					APICall(fmDate.format(this.userPreferences.getDays().getLast().getDate()),fmTime.format(this.userPreferences.getDays().getLast().getLastUpdated()),fmTime.format(currentTime),this.userPreferences.getDays().getLast());
				}
...
//					mainWindow.getDays().add(day);
					...
					Day day = new Day(new Date(this.userPreferences.getFutureDays().getLast().getDate().getTime() + 24*60*60*1000));	//Has a date that is one day ahead of the last day in "futureDays"
				...
				{ 
					...
					APICall(fmDate.format(this.userPreferences.getDays().getLast().getDate()),fmTime.format(this.userPreferences.getDays().getLast().getLastUpdated()),"23:59",this.userPreferences.getDays().getLast()); 												
					...
					if (nDays == 0) 													//only new current day needs to be updated
					{
						...
						APICall(fmDate.format(this.userPreferences.getDays().getLast().getDate()),"00:00",fmTime.format(currentTime),this.userPreferences.getDays().getLast()); 
					}
					else if(nDays > 0)
					{
						...
						{ 
							...
							APICall(fmDate.format(this.userPreferences.getDays().getLast().getDate()),"00:00","23:59",this.userPreferences.getDays().getLast()); 
						}
